...
COMMIT_STATUS_PATTERN = re.compile(
    r"(AWS CodeBuild (BuildBatch )?us-west-[1-2] )?\(?(?P<context>[\w\s-]+)\)?"
)
...
def review_release(app_repo, app_dir_path, release_git_ref):
    ...
    if curr_splunk_supported and new_release_splunk_supported:
        ...
        logging.info(
            "Newly %s supported supported app that was previously Splunk supported requires manual review",
    ...
    if missing_statuses:
        logging.warning(
            "The following required commit statuses are missing for release %s - %s: %s",
            release_git_ref,
            commit.sha,
            missing_statuses,
        )
    ...
    if unsuccessful_statuses:
        logging.warning(
            "The following required commit statuses are unsuccessful for release %s - %s: %s",
            release_git_ref,
            commit.sha,
    ...
    if manual_approval:
        logging.warning(
            "The release of %s %s requires manual approval", app_repo.name, release_git_ref
        )
        return 1
