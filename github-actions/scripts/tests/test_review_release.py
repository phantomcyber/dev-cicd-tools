...
def app_json(request):
    return {"appid": "123", "app_version": "1.0", "publisher": request.param}
...
def test_review_release_auto_approved(
    github_repo, app_json, combined_commit_status, commit_status_names
):
    if app_json["publisher"] == "Splunk":
        pytest.skip()

    mock_statuses = [mock_commit_status(name, "success") for name in commit_status_names]
...
def test_review_release_manual_approval_splunk_supported_app(
    github_repo, app_json, combined_commit_status, commit_status_names
):
    if app_json["publisher"] != "Splunk":
        pytest.skip()
...
def test_review_release_manual_approval_missing_commit_statuses(
    github_repo, app_json, combined_commit_status, commit_status_names
):
    mock_statuses = [mock_commit_status(name, "success") for name in commit_status_names[:-1]]
    combined_commit_status.statuses = mock_statuses
...
def test_review_release_manual_approval_unsuccessful_commit_statuses(
    github_repo, app_json, combined_commit_status, commit_status_names
):
    mock_statuses = [mock_commit_status(name, "failed") for name in commit_status_names]
    combined_commit_status.statuses = mock_statuses
...
def test_review_release_manual_approval_support_status_change(
    github_repo, app_json, combined_commit_status, commit_status_names
):
    mock_statuses = [mock_commit_status(name, "success") for name in commit_status_names]
    combined_commit_status.statuses = mock_statuses
