...
COMMIT_AUTHOR = {"name": "root", "email": "root@splunksoar"}
...
def deserialize_text_file(unreleased_md):
    return base64.b64decode(unreleased_md["content"]).decode(DEFAULT_ENCODING).split("\n")
...
def start_release(session, app_dir: Optional[str] = None):
    ...
    json_files = [
        f["name"]
        for f in repo_files
        if not f["name"].lower().endswith(".postman_collection.json")
        and f["name"].lower().endswith(".json")
    ]

    if len(json_files) != 1:
        raise ValueError(f"Expected a single json file in the repo but got {json_files}")
    ...
    sha_field = session.get(f"contents/{app_json_file}?ref=next")["sha"]
    app_json_next, app_json_indent = deserialize_app_json(
        session.get(f"git/blobs/{sha_field}?ref=next")
    )
    app_version_next = app_json_next["app_version"]

    try:
        sha_field = session.get(f"contents/{app_json_file}?ref=main")["sha"]
        app_json_main, _ = deserialize_app_json(session.get(f"git/blobs/{sha_field}?ref=main"))
    ...
    if LooseVersion(app_version_next) <= LooseVersion(app_version_main):
        logging.warning(
            "Detected the app version in next %s to be <= the app version in main %s",
            app_version_next,
            app_version_main,
        )

        app_json_next["app_version"] = app_version_next = app_version_main[:-1] + str(
            int(app_version_main[-1]) + 1
        )
    ...
    for path, content in updates.items():
        tree.append(
            {
                "sha": session.post("git/blobs", content=content, encoding=DEFAULT_ENCODING)["sha"],
                "path": path,
                "mode": "100644",
                "type": "blob",
            }
