...
from start_release import (
    start_release,
    RELEASE_NOTES_DATE_FORMAT,
    DEFAULT_ENCODING,
    RELEASE_NOTE_COMMIT_MSG,
    COMMIT_AUTHOR,
    load_main_pr_body,
    MAIN_PR_TITLE,
    deserialize_app_json,
    FIRST_VERSION,
)
...
TestData = namedtuple("TestData", ["unreleased_md", "expected_release_notes_md"])


@pytest.fixture(
    scope="function",
    params=[TestData("tests/data/unreleased.md", "tests/data/expected_release_notes.md")],
)
...
def encode_text_file(file_path):
    ...
    with open(file_path) as f:
        return {"content": base64.b64encode(f.read().encode(DEFAULT_ENCODING))}
...
def test_start_release_happy_path(
    session,
    next_version,
    main_version,
    ...
    app_json_main = (
        mock_app_json(main_version) if main_version else HTTPError(response=Mock(status_code=404))
    )

    sha_field_next = SHA
    sha_field_main = SHA

    open_prs = [{"head": {"ref": "a"}, "base": {"ref": "b"}, "html_url": "url"}]
    if existing_pr:
        open_prs.append({"head": {"ref": "next"}, "base": {"ref": "main"}, "html_url": "url"})
    if isinstance(app_json_main, dict):
        session.get.side_effect = [
            [{"name": f"{APP_NAME}.json"}, {"name": f"{APP_NAME}.postman_collection.json"}],
            app_json_next,
            app_json_next,
            app_json_main,
            app_json_main,
    ...
    if LooseVersion(next_version) <= LooseVersion(main_version):
        ...
        expected_tree = [(f"{APP_NAME}.json", json_sha)]
    else:
    ...
    if isinstance(app_json_main, dict):
        ...
        assert session.get.call_args_list[1] == call(f"contents/{APP_NAME}.json?ref=next")
        assert session.get.call_args_list[2] == call(f"git/blobs/{sha_field_next}?ref=next")
        assert session.get.call_args_list[3] == call(f"contents/{APP_NAME}.json?ref=main")
        assert session.get.call_args_list[4] == call(f"git/blobs/{sha_field_main}?ref=main")
    ...
    for i, blob in enumerate(expected_blobs):
        assert session.post.call_args_list[i] == call(
            "git/blobs", content=blob, encoding=DEFAULT_ENCODING
        )

    post_idx = len(expected_blobs)
    expected_tree = [
        {"mode": "100644", "type": "blob", "path": path, "sha": sha} for path, sha in expected_tree
    ]
    assert session.post.call_args_list[post_idx] == call(
        "git/trees", tree=expected_tree, base_tree=base_sha
    )
    post_idx += 1
    ...
    post_idx += 1
    if not existing_pr:
        assert session.post.call_args_list[-1] == call(
            "pulls",
            head="next",
            base="main",
...
@pytest.fixture(
    scope="function", name="session_invalid_data", params=[[zero_json_files], [multiple_json_files]]
)
