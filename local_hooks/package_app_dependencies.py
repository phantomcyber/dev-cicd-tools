...
def _update_app_json(app_json, pip_dependencies_key, wheel_entries, app_dir):
    ...
    """
    wheel_paths = [
        {"module": w.module, "input_file": w.input_file}
...
def _parse_pip_dependency_wheels(app_json, pip_dependency_key):
    ...
    return [
        AppJsonWheelEntry(w["module"], w["input_file"]) for w in pip_dependencies.get("wheel", [])
    ]
...
def _copy_new_wheels(new_wheels, new_wheels_dir, app_dir):
    ...
    for whl in new_wheels:
        for cp_tag in CPythonTag:
            ...
            raise ValueError(
                f"{whl.file_name} has an unexpected python version tag: {whl.python_version}"
            )
...
def _remove_unreferenced_wheel_paths(
    app_dir, existing_wheel_paths, new_wheel_paths, wheel_entries_for_other_py_versions
):
    """
    Removes wheels from the app directory that will no longer be referenced by in app JSON.
    """
    all_referenced_wheel_paths = set(
        itertools.chain(
...
def main():
    ...
    try:
        ...
        all_built_wheels = set(
            Wheel(
                m.group(), m.group("distribution"), m.group("python_version"), m.group("platform")
            )
        ...
        wheels_for_other_py_versions = list(
            itertools.chain.from_iterable(
                _parse_pip_dependency_wheels(app_json, pip_dep.value)
                for pip_dep in PipDependency
                if pip_dep.value != pip_dependencies_key
            )
        )

        _remove_unreferenced_wheel_paths(
            app_dir=app_dir,
