...
if ! "${PY313_BIN}/python" --version &>/dev/null; then
	...
	exec docker run \
		--rm \
		-v "$APP_DIR":/src \
		-v "$(dirname "$0")":/srv/local_hooks \
		-v "$PY_SITE":/site-packages \
		-e "PYTHONPATH=/srv:/site-packages" \
		-w /src \
		"$IMAGE" \
		/bin/bash -c "/srv/local_hooks/$script_name"
fi

export PATH="$PY39_BIN:$PY313_BIN:$PATH"

HOOKS_PYTHONPATH="${PYTHONPATH:-}"
PACKAGER_PYTHON="${PY313_BIN}/python"

# Sanity check: We can import local_hooks, right? If not, try the source mount path used by
# the Docker wrapper.
if ! "$PACKAGER_PYTHON" -c 'import local_hooks'; then
	HOOKS_PYTHONPATH="/srv:${HOOKS_PYTHONPATH}"
	export PYTHONPATH="$HOOKS_PYTHONPATH"
	"$PACKAGER_PYTHON" -c 'import local_hooks'
fi
...
SCRIPT=("$PACKAGER_PYTHON" -m local_hooks.package_app_dependencies)
...
has_pip_dependency_key() {
	...
	if [[ -n "${HOOKS_PYTHONPATH:-}" ]]; then
		PYTHONPATH="$HOOKS_PYTHONPATH" "$PACKAGER_PYTHON" -c "$check_dependency_key" "$dependency_key"
	else
		"$PACKAGER_PYTHON" -c "$check_dependency_key" "$dependency_key"
	fi
}
